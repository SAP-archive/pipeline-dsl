...
class Task:
    def __init__(self, name, jobname, timeout, image_resource, script, inputs, secrets):
        ...
        self.config = {
            ...
            "params": dict(map(lambda kv: (kv[1], '(({}))'.format(kv[1])), secrets.items())),
            "run": {
...
class InitTask:
    ...
    def concourse(self):
        ...
        files = list(map(lambda x: x[skip:], glob.glob(
            os.path.join(self.script_dir, '*.py'), recursive=True)))
        cmd = '{tar} cj --sort=name --mtime="UTC 2019-01-01" --owner=root:0 --group=root:0 -b 1 -f - -C {script_dir} {files}'.format(
            tar=tar, script_dir=self.script_dir, files=" ".join(files))
...
class Pipeline():
    ...
    def job(self, name):
        result = Job(name, self.script, self.image_resource,
                     self.resource_chains)
    ...
    def main(self):
        ...
        if args.concourse:
            ...
            return self.run_task(args.job, args.task)
        else:
            return self.run()
